...
class TextSnipper:
    TesseractPath: str = (
        # r"C:\My Program Files\Tesseract-OCR\tesseract.exe"  # path to tesseract.exe
        "/usr/bin/tesseract"
    )
    ...
    def __init__(self, mode: str, preprocess_modes: list[str]) -> None:
        ...
        self.preprocess: dict[str, Callable] = {
            "-g": self.get_grayscale,
            "-r": self.remove_noise,
            "-t": self.thresholding,
            "-d": self.dilate,
            "-e": self.erode,
        ...
        self.preprocess_modes: list[str] = (
            preprocess_modes if preprocess_modes[0] != "" else []
        )

        self.models: dict[str, Callable] = {
            "easyocr": self.easyocr_mode,
            "tesseract": self.tesseract_mode,
        }
        self.model: str = mode
    ...
    def thresholding(self) -> np.ndarray:
        ...
        return cv2.threshold(
            self.image_array, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
        )[1]
